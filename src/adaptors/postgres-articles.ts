...
export default class PostgresArticles implements Articles {
  ...
  async get(id: NamedNode): Promise<DatasetCore> {
    const article = await this.database.oneOrNone('SELECT article FROM articles WHERE uuid = $[uuid]', { uuid: termToString(id) }, (data: { article: string } | null) => (
  ...
  async* [Symbol.asyncIterator](): AsyncIterator<[NamedNode, DatasetCore]> {
    yield* await this.database.any('SELECT uuid, article FROM articles').then((rows) => rows.map(async (row: { uuid: string; article: string }) => [stringToTerm(row.uuid), await fromStream(this.dataFactory.dataset(), this.parser.import(toReadableStream(row.article)))])) as [[NamedNode, DatasetCore]];
  }
