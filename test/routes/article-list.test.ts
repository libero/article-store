...
const makeRequestEmptyLst = async (next?: Next): Promise<Response> => {
...
const makeRequestList = async (next?: Next): Promise<Response> => {
  const context = createContext();
  const articles = new InMemoryArticles();
...
describe('article list', (): void => {
  it('should return a successful response', async (): Promise<void> => {
    const response = await makeRequestEmptyLst();
  ...
  it('should call the next middleware', async (): Promise<void> => {
    const next = jest.fn();
    await makeRequestEmptyLst(next);
